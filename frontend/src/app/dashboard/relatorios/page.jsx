...
import { getAlunos, getTurmas, getFrequenciaAluno } from "@/lib/api";
import {
...
function RelatoriosContent() {
  ...
  useEffect(() => {
    ...
    fetchFrequenciasAlunos(alunos)
      ...
      .catch(() => {
        if (!cancelled) setAlunosComFreq(alunos.map((a) => ({ ...a, percentualPresenca: null })));
      })
      .finally(() => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2 border border-neutral-200 rounded-lg text-sm opacity-50 cursor-not-allowed"
          >
          ...
          <button
            type="button"
            disabled
            title="Exportação Excel ainda não disponível"
            className="inline-flex items-center gap-1.5 px-3 py-2 border border-neutral-200 rounded-lg text-sm opacity-50 cursor-not-allowed"
          >
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between px-5 py-4 border-b border-bg-border">
                  <div>
                    ...
                    <p className="text-caption text-neutral-500">
                      {t.presencas ?? 0} presenças · {t.faltas ?? 0} faltas · {t.totalAulas ?? t.historico?.length ?? 0} aula(s)
                    </p>
                  </div>
                ...
                {(!t.historico || t.historico.length === 0) && (
                  <p className="text-center py-6 text-neutral-500 text-sm">Nenhuma aula registrada ainda.</p>
                )}
              </div>
            );
          })}
          {freqTurmas.length === 0 && (
            <p className="text-center py-10 text-neutral-500">Nenhuma disciplina com aulas registradas.</p>
          )}
        </div>
      )}
